...
module.exports = {
  config: {
    ...
    longDescription: {
      en: 'This command fetches a random inspirational quote.',
    },
    category: '𝗕𝗢𝗫',
    guide: {
      en: '{p}motivation on / off',
    },
  },
  onStart: async function ({ api, event, args }) {
    ...
    if (command === 'on') {
      if (isMotivationEnabled) {
        api.sendMessage('Motivation feature is already enabled.', event.threadID);
      } else {
        isMotivationEnabled = true;
        api.sendMessage('Motivation feature is now enabled. Quotes will be sent every minute.', event.threadID);
        startSendingMotivationalQuotes(api, event.threadID);
      }
    } else if (command === 'off') {
      if (!isMotivationEnabled) {
        api.sendMessage('Motivation feature is already disabled.', event.threadID);
      } else {
        isMotivationEnabled = false;
        clearInterval(intervalId);
        api.sendMessage('Motivation feature is now disabled.', event.threadID);
      }
    } else {
...
async function startSendingMotivationalQuotes(api, threadID) {
  ...
  }, 60 * 1000); // Send a quote every 1 minute
}
